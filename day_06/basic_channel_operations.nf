...
workflow {
    ...
    // Task 7 - Use fromPath to include all fastq files in the "files_dir" directory,
    ...
    if (params.step == 8) {
        ...
        in_ch = ch_1.mix(ch_2, out_ch).view()
    }
    ...
    // Task 11 -  From the input channel, create lists where each first item in the
    // list of lists is the first item in the output channel, followed by a list of
    // all the items its paired with
    // e.g. 
    // in: [[1, 'A'], [1, 'B'], [1, 'C'], [2, 'D'], [2, 'E'], [3, 'F']]
    // out: [[1, ['A', 'B', 'C']], [2, ['D', 'E']], [3, ['F']]]
    if (params.step == 11) {
        in_ch = channel.of([1, 'V'], [3, 'M'], [2, 'O'], [1, 'f'],
                           [3, 'G'], [1, 'B'], [2, 'L'], [2, 'E'],
                           [3, '33'])
                .groupTuple()
                .view()
    ...
    if (params.step == 12) {
        ...
        in_ch = left_ch.join(right_ch, remainder: true).view()
    }

    // Task 13 - Split the input channel into two channels, one of all the even numbers
    // and the other of all the odd numbers. Write the output of each channel to a list
    ...
    if (params.step == 13) {
        ...
        in_ch.even.collect().view { "Even: $it" }
        in_ch.odd.collect().view { "Odd: $it" }
    }

    // Task 14 - Nextflow has the concept of maps. Write the names in the maps in
    // this channel to a file called "names.txt". Each name should be on a new line.
    ...
    if (params.step == 14) {
        ...
        .collectFile (storeDir: './results') {it -> ['names.txt', it.name + '\n']}
    }
}